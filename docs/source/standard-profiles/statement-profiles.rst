...
In the **GKS-Core-IM**, each discrete assertion of knowledge is captured in a self-contained ``Statement`` object which roots a data structure supporting rich and flexible descriptoins of the evidence and provenance supporting this knowledge (see `here <https://va-ga4gh.readthedocs.io/en/latest/modeling-foundations.html#core-im-statement-structure>`_).

**Statement Profiles** are defined as specializations of this Core-IM ``Statement`` class, to provide a concrete schema for representing a particular type of Variant knowledge (e.g. a variant pathogenicity classification).

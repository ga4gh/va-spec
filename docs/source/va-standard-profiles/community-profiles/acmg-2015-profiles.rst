...
Profiles are authored as YAML-based specifications defining **ACMG-based constraints** on top of a core class definitions from which machine-readable JSON Schema and user-facing Web Documentation are automatically generated by `GKS Metaschema Tooling <https://github.com/ga4gh/gks-metaschema>`_.
...
An Evidence Line that describes how evidence for a variant was interpreted to determine if a specific ACMG 2015 criterion code is met, and the strength that evidence this provides for or against the variant's pathogenicity. 

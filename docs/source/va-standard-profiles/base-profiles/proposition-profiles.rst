...
The ``subject``, ``predicate``, ``object`` attributes defined in this profile can be used to create structured representations of basic propositions such as  *"NP_000305.3:p.Arg11Thr impacts the function of the human PTEN gene"*. Here, an optional ``experimentalContextQualifier`` attribute can reference an assay, study, or analysis in which the reported impact was observed, which can further inform interpretation of the core proposition. These semantics were defined to support the needs of an early :ref:`MAVE-DB implementation <implementations>` of the VA-Spec, and will likely be refined and/or expanded as additional adopters and use cases emerge.

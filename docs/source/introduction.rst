...
VA-Spec defines a set of schema for representing different types of knowledge about genetic variants, to provide a common format for exchange between data systems. Each schema is built as a "Profile" that extends a common, domain-agnostic :ref:`VA Core Model <va-core-model>`. The initial v1 release of the VA-Spec includes the Profiles descried :ref:`va-profiles>`here, which support specific types of variant knowledge produced by implementing Driver Projects and organizational members, including `ClinGen <https://clinicalgenome.org/>`_, `VICC <https://cancervariants.org/index.html>`_, and the `Atlas of  Variant Effects Alliance <https://www.varianteffect.org/>`_. These Profiles are provided as machine-readable JSON Schema specifications that support sharing and validation within and across these projects, along with a python reference implementation. 
...
VA-Spec is intended to complement other GA4GH exchange formats such as `VRS <https://vrs.ga4gh.org/en/latest/index.html>`_  and `Cat-VRS <https://cat-vrs.readthedocs.io/en/latest/index.html>`_, which support interoperable representation of the variation concepts tha are the subjects of variant annotations.
...
The initial release includes the following :ref:`VA Profiles <va-profiles>`, provided as JSON schema for use by the GA4GH community:
